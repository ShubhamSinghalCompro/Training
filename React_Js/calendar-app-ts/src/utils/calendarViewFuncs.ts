 ...
 export const doesEventOverlapWithInterval = (event: Event, interval: Date) => {
  debugger
    ...
    const ans = eventEnd >= 1380 ? ( eventStart < 1440 && eventEnd > intervalStart ) : (eventStart < nextIntervalStart && eventEnd > intervalStart);
  ...
  export const calculateEventPositionInInterval = (event: Event, interval: Date) => {
    ...
    const visibleEnd = eventEnd >= 1380 ? eventEnd :Math.min(eventEnd, nextIntervalStart);
